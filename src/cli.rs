...
fn collect_frame_paths(frames_dir: &Path) -> Result<Vec<PathBuf>> {
	...
	for entry in entries {
		let entry = entry.map_err(|e| QrodeError::Internal(format!("failed to read dir entry: {e}")))?;
		let path = entry.path();
		if !path.is_file() {
			continue;
		}
...
pub fn run(args: CliArgs) -> Result<()> {
	...
	if is_frames_target {
		...
		if frames.is_empty() {
			return Err(QrodeError::Internal("frame_limit resulted in zero frames to process".into()));
		}
		...
		| -> Result<Vec<u8>> {
			...
			if args.write_json {
				let file = File::create(&frame_json)
		...
		while idx < frame_jobs.len() {
			...
			if let Some((source_idx, payload)) = exact_reuse_payload {
				...
				let payload = run_frame(&frame_jobs[idx], Some(payload), Some(source_idx), Some(0.0))?;
