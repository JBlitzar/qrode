...
use crate::qr_core::{
    EncodedQr, MaskPattern, QrSpec, QrodeError, Result, capacity_bytes, encode_with_mask,
    immutable_mask,
};
...
fn mutate_payload(
    ...
    if rng.gen_bool(cfg.block_mutation_prob as f64) {
        ...
        for (offset, b) in candidate[block_start..block_start + block_len]
            .iter_mut()
            .enumerate()
        {
...
fn run_single_restart(
    ...
    let (best_enc, best_score) = image_space_perturb(
        best_enc, best_score, input, cfg, eval_ctx, &mut rng, stop_at,
    )?;
        ...
        for _ in 0..cfg.coordinate_refine_sweeps {
            ...
            for byte_idx in &indices {
                ...
                for candidate in allowed {
                    ...
                    proposal[*byte_idx] = *candidate;
                    let (proposal_enc, proposal_score) =
                        evaluate_payload(&proposal, cfg, input, eval_ctx)?;
                    if proposal_score.objective_score > best_local_score.objective_score {
                        best_byte = *candidate;
                        best_local_score = proposal_score;
